            ...
            <div class="row">
                <div class="col-lg-12">
                        <div class="text-right">
                            <a href="{{ route('settings.slider.create') }}" type="button" class="btn btn-primary">Добавить слайдер</a>
                        </div>
                </div>
            </div>
                ...
                <table class="table datatable-column-search-inputs">
                    ...
                    @foreach($pages as $page)
                        <tr @if($page->is_published == 0) class="not-active" @endif>
                            <td><a href="{{ route('settings.slider.show', $page->id) }}">{{ $page->h1 }}</a></td>
                            <td>{{ $page->created_at }}</td>
                            <td><a href="{{ route('settings.slider.edit', $page->id) }}"><i class="icon-pencil3"></i></a></td>
                            <td>
                                <form action="{{ route('settings.slider.destroy', $page->id) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
                                    <div class="text-right">
