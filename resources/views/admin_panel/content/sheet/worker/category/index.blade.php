...
@section('pageheader-title')
    <a href="{{ route('content.sheet.worker.index') }}"><i class="icon-arrow-left52 mr-2"></i></a>Подразделения
@endsection
@section('header-js')
            ...
            <div class="row">
                <div class="col-lg-12">
                        <div class="text-right">
                            <a href="{{ route('content.sheet.worker.category.create') }}" type="button" class="btn btn-primary">Добавить подразделение</a>
                        </div>
                </div>
            </div>
            <div class="row">
            ...
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Список подразделений</h5>
                    </div>
                    <form action="{{ route('content.sheet.worker.category.orderrenew') }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <div class="card-body">
                                    ...
                                    @if($categoryes->count() > 0)
                                        @foreach($categoryes as $category)
                                            <li class="p-2 bg-light border rounded cursor-move mt-1 ui-sortable-handle" style="">
                                                ...
                                                <input type="hidden" name="content_sheet_worker_category_id[]" value="{{ $category->id }}">
                                            </li>
                                        @endforeach
                                    @endif
                                </ul>
